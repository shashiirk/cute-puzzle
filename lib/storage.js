...
async function saveLocal(imageSrc, gridSize, message) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ id, imageSrc, gridSize, message: message || '', createdAt: new Date().toISOString() }),
    'utf8'
  )
  return id
}
...
async function saveVercel(imageUrl, gridSize, message) {
  const { put } = await import('@vercel/blob')

  const id = nanoid(10)
  const meta = { id, imageUrl, gridSize, message: message || '', createdAt: new Date().toISOString() }
...
// saveChallenge(imageData, gridSize, message?)
